...
public abstract class StructuralRelationsImporter extends RelationsImporter {
  ...
  protected StructuralRelationsImporter(String taskName, SynchronizedUnknownsMap unknowns) {
    super(taskName);
  ...
  protected final void insert(ReaderBundle reader, Integer projectID) {
    ...
    Collection<RelationEX> newTypeRelations = insertRemainingEntities(reader, projectID);
    loadRemainingEntityMap(projectID);
    insertStructuralRelations(reader, newTypeRelations, projectID);
    
    fileMap.clear();
    entityMap.clear();
  }
  
  private Collection<RelationEX> insertRemainingEntities(ReaderBundle reader, Integer projectID) {
    ...
    TypeEntityResolver resolver = new TypeEntityResolver(inserter, projectID);
  ...
  private void insertStructuralRelations(ReaderBundle reader, Collection<RelationEX> newTypeRelations, Integer projectID) {
      ...
      for (LocalVariableEX var : reader.getTransientLocalVariables()) {
        if (result.next()) {
          ...
          if (var.getType() == LocalVariable.PARAM) {
            entityMap.put(var.getParent() + "#" + var.getPosition(), DatabaseEntity.make(entityID, RelationClass.INTERNAL));
          }
  ...
  private class TypeEntityResolver {
    ...
    public TypeEntityResolver(BatchInserter inserter, Integer projectID) {
