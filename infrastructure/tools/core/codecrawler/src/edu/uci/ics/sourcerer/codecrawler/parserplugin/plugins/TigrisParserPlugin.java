 ...
 * 
 */
public class TigrisParserPlugin extends ParserPlugin {
	...
	public TigrisParserPlugin() {
		String keywordsValue = CrawlerProperties.getInstance().getProperty(
				CrawlerProperties.CRAWLER + ".parserplugin.TigrisParserPlugin.keywords",
	...
	private Set<Hit> processDocumentList(Parser htmlParser, String referringUrl) {
		...
		for (DocumentListTableRow row : tableRows) {
			...
			//we're taking only .zip or .jar files
			if (!(row.fileUrl.toLowerCase().endsWith(".zip") || row.fileUrl.toLowerCase().endsWith(".jar")))
				continue;
			...
			if (versionMatcher.find()) {
				hit.setVersion(CertaintyUtils.addCertaintyToValue(
						row.fileName.substring(versionMatcher.start(), versionMatcher.end()),
						0.9D));
			} else if (headingVersion != null)
				hit.setVersion(CertaintyUtils.addCertaintyToValue(headingVersion, 0.9D));
	...
	private Set<Hit> processSourceBrowse(Parser htmlParser, String referringUrl) {
		...
		Hit hit = new Hit(getIdGenerator().getNewHitId(),
				"cvs -d :pserver:guest@cvs.tigris.org:/cvs login;" +
				"cvs -d :pserver:guest@cvs.tigris.org:/cvs checkout " + 
